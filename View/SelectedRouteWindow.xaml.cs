...
{
    public partial class SelectedRouteWindow : Window
    {
        private CertainRoute Route { get; set; }
        DataBaseAdapter dataBaseAdapter = new DataBaseAdapter();
        private ObservableCollection<Carriage> carriages = new ObservableCollection<Carriage>();

        public SelectedRouteWindow(CertainRoute route)
        {
            InitializeComponent();
        ...
        {
            DeparturePointTextBlock.Text = Route.DeparturePoint;
            DestinationPointTextBlock.Text = Route.DestinationPoint;
            DepartureDateTextBlock.Text = Route.DateTimeDeparture;
            DestinationDateTextBlock.Text = Route.DateTimeArrival;
            TrainIDTextBlock.Text = Route.TrainID.ToString();
            ...
            try
            {
                ...
                DataTable tempTable = await dataBaseAdapter.GetTableAsync($"SELECT Carriage.ID, Type_Carriage.Name FROM Carriage JOIN Type_Carriage ON Carriage.ID_Type_Carriage = Type_Carriage.ID JOIN Train ON Carriage.ID_Train = Train.ID WHERE Train.ID = {Route.TrainID}");
            ...
            {
                MessageBox.Show("Не удалось получить данные из базы данных или что-то другое {ex}", "Ошибка");
            }
        }
        ...
        private async Task<double> CalculatePrice()
        {
            ...
            try
            {
                if (CarriageBox.SelectedItem != null && TicketRateTypeBox != null)
                {
                    ...
                    routePrice += Convert.ToDouble(await dataBaseAdapter.GetCellAsync($"SELECT Route.Price FROM Route JOIN City AS Depart ON Depart.ID = Route.ID_City_Of_Departure JOIN City AS Destin ON Destin.ID = Route.ID_Destination_City WHERE Depart.Name = '{Route.DeparturePoint}' AND Destin.Name = '{Route.DestinationPoint}'"));
                }
                else
                    MessageBox.Show("Нужно выбрать все параметры для подсчета цены", "Внимание");
            }
        ...
        private async void IssueTicket_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (CarriageBox.SelectedItem != null && TicketRateTypeBox != null && SeatsComboBox.SelectedItem != null)
            {
                ...
                try
                {
                    ...
                    int scheduleID = Convert.ToInt32(await dataBaseAdapter.GetCellAsync($"SELECT Schedule.ID FROM Schedule WHERE ID_Train = {Route.TrainID}"));
                    ...
                    await dataBaseAdapter.ExecuteQueryAsync($"INSERT INTO [Ticket] (ID_Schedule, ID_User, ID_Passenger, ID_Ticket_Rate, Carriage_Number, Seat_Number) VALUES ({scheduleID}, {User.ID}, {passengerID}, {ticketRateID}, {carriages[CarriageBox.SelectedIndex].CarriageID}, {SeatsComboBox.SelectedValue})");
        ...
        private void CreatePDFDoucment()
        {
            ...
            page.AddText($"Поезд: {Route.TrainID}  Вагон: {carriages[CarriageBox.SelectedIndex].CarriageID}  Место: {SeatsComboBox.SelectedValue.ToString()}", 14, new PdfPoint(25, 770), font);
            ...
            page.AddText($"Место отправления: {Route.DeparturePoint}", 14, new PdfPoint(25, 690), font);
            page.AddText($"Дата и время отправления: {Route.DateTimeDeparture}", 14, new PdfPoint(25, 670), font);
            page.AddText($"Место прибытия: {Route.DestinationPoint}", 14, new PdfPoint(25, 640), font);
            page.AddText($"Дата и время прибытия: {Route.DateTimeArrival}", 14, new PdfPoint(25, 620), font);
            ...
            File.WriteAllBytes($@"{path}\Ticket-{Route.DeparturePoint}-{Route.DestinationPoint}-{date.ToString("dd-MM-yyyy-hh-mm")}.pdf", documentBytes);
        }
